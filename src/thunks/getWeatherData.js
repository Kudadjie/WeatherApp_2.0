  ...
  return async (dispatch) => {
    ...
    setTimeout(async () => {
      const response =
          await axios
              .get(
                  `https://api.weatherapi.com/v1/forecast.json?key=c1cc53b5ba214351bc9232844221703&q=${
                      locationCoordinates}&days=1&aqi=no&alerts=no`)
              .then((res) => { return res; })
      ...
      dispatch(setDay({
        value : Boolean(
            response.data.forecast.forecastday[0]
                .hour[Number(
                    String(response.data.location.localtime).match(/\s\d+/))]
                .is_day),
      }));
      dispatch(setRiseSetValues({
        sun : `${response.data.forecast.forecastday[0].astro.sunrise} | ${
            response.data.forecast.forecastday[0].astro.sunset}`,
        moon : `${response.data.forecast.forecastday[0].astro.moonrise} | ${
            response.data.forecast.forecastday[0].astro.moonset}`,
      }));
      dispatch(setTempValues({
      ...
      setTimeout(() => { dispatch(updateLastUpdated({time : "30 mins ago"})); },
                 1.8e6);
